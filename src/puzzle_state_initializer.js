import initialBlocksVertices from './initial_blocks_vertices.js';
...
class PuzzleSolutionInitializer {
  static createInitialPuzzleSolution(blockToFinalPositionMapping, blockToOrientationMapping) {
    ...
    _.forEach(blockToFinalPositionMapping, function(blockIndex, finalPositionIndex) {
      ...
      PuzzleSolutionInitializer.orientBlock(block, finalPosition, blockToOrientationMapping[blockIndex]);
  ...
  static orientBlock(block, finalPosition, orientation) {
    const finalPositionCenter = PuzzleSolutionInitializer.calculateFinalPositionCenter(finalPosition);
    const translationToOrigin = _.map(finalPositionCenter, coord => {
      return -coord;
    })
